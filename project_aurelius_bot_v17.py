...
# v10 - CANLI MOD icin OZEL/IMZALI uc noktalar.
# !!! ONEMLI: Bu adresler kullanicidan alinan yol yapisina (/open-api/v3/...)
# gore olusturulmustur. Binance TR'nin ozel API'si icin genis capli, resmi
# indekslenmis dokumantasyon bulunmamaktadir - CANLI_MOD=True ile
# calistirmadan once bu adresleri MUTLAKA kendi Binance TR API
# dokumantasyonunuzdan dogrulayin ve gerekirse asagidaki iki sabiti
# guncelleyin.
BINANCE_TR_PRIVATE_BASE_URL = "https://www.binancetr.com"
ACCOUNT_ENDPOINT_PATH = "/open-api/v3/account"
ORDER_ENDPOINT_PATH = "/open-api/v3/order"
RECV_WINDOW_MS = 5000
...
# v10 - Ortam degiskenlerinden okunan kimlikler
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")
BINANCE_TR_API_KEY = os.getenv("BINANCE_TR_API_KEY", "")
BINANCE_TR_SECRET_KEY = os.getenv("BINANCE_TR_SECRET_KEY", "")
...
def _binance_imzali_sorgu(params: dict) -> str:
    """RFC 2104 HMAC-SHA256 imzali query string olusturur."""
    params = dict(params)
...
def binance_signed_request(method: str, path: str, params: Optional[dict] = None):
    """
    v10: CANLI MOD icin RFC 2104 HMAC-SHA256 imzali istek gonderir.
    X-MBX-APIKEY header'i ile birlikte /open-api/v3/account (GET) veya
    /open-api/v3/order (POST) gibi ozel uc noktalara istek atar.
    """
    query_signed = _binance_imzali_sorgu(params or {})
    headers = {"X-MBX-APIKEY": BINANCE_TR_API_KEY, "User-Agent": "grid-bot-sim"}
...
def binance_serbest_try_bakiyesi() -> float:
    """v10: /open-api/v3/account uzerinden GERCEK serbest TRY bakiyesini ceker."""
    veri = binance_signed_request("GET", ACCOUNT_ENDPOINT_PATH)
    for bakiye in veri.get("balances", []):
        if bakiye.get("asset") == "TRY":
            return float(bakiye.get("free", 0.0))
    return 0.0
...
def mutabakat_yap(kasa: "MerkeziKasa", pozisyonlar: dict) -> None:
    ...
    /open-api/v3/account'tan GERCEK serbest TRY bakiyesini VE acik
...
def binance_gercek_emir_gonder(symbol: str, side: str, quantity: float) -> dict:
    """v10: /open-api/v3/order uc noktasina GERCEK MARKET emri gonderir
    (HMAC-SHA256 imzali). side: 'BUY' veya 'SELL'. LOT_SIZE/minNotional
    kontrolu cagiran taraf (CoinBot._canli_emir_dogrula_ve_gonder)
    tarafindan ONCEDEN yapilmis olmalidir. Bu fonksiyon partial-fill
    (kismi gerceklesme) durumunu ayrica sorgulamaz - tek seferlik MARKET
    emri gonderir ve borsanin dondurdugu yaniti aynen dondurur."""
    params = {
        "symbol": symbol,
        "side": side,
        "type": "MARKET",
...
def canli_mod_on_kontrol() -> bool:
    """v10: CANLI_MOD=True ile baslatilmadan once gerekli TUM guvenlik
    kontrollerini yapar. Herhangi biri eksikse bot CANLI baslamaz."""
    if not CANLI_MOD:
        return True

    hata_var = False
    if not BINANCE_TR_API_KEY or not BINANCE_TR_SECRET_KEY:
